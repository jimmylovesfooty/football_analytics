...
def read_simple_event_data(game_id):
    '''
    read_event_data(game_id):
    read Metrica event data for game_id and return as a DataFrame
    '''
    eventfile = f'Sample_Game_{game_id}_RawEventsData.csv'  # filename
    events = pd.read_csv(eventfile)  # read data from current directory
    return events
...
def easy_tracking_data(game_id, side):
    '''
    tracking_data(game_id, side):
    Read Metrica tracking data for game_id and return as a DataFrame.
    "side" refers to the team side: either 'Home' or 'Away', as found in the filename.
    '''
    # Construct the correct filename based on the game ID and side ('Home' or 'Away')
    teamfile = f'Sample_Game_{game_id}_RawTrackingData_{side}_Team.csv'
    
    # First: deal with file headers to get the player names correct
    with open(teamfile, 'r') as csvfile:  # create a CSV file reader
        reader = csv.reader(csvfile)
        teamnamefull = next(reader)[3].lower()
        ...
        for i, j in enumerate(jerseys):
            columns[i*2+3] = f"{side}_{j}_x"  # Use 'Home' or 'Away' instead of team name
            columns[i*2+4] = f"{side}_{j}_y"
        
        # Column headers for the x & y positions of the ball
        columns[-2] = "ball_x"
        columns[-1] = "ball_y"
    
    # Second: read in tracking data and place it into a pandas DataFrame
